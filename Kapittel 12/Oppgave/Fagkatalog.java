...
class Fagkatalog {
  ...
  public int finnAntallStudenter() {
    int antStudenter = 0;
      for (int i = 0; i < antFag; i++) { //Går gjennom alle fagene
          antStudenter += fagene[i].getAntStud();
      }

    return antStudenter;
  }

  public String[] finnFlestStud() {
    int hittilStorst = fagene[0].getAntStud();
    String[] storst = {fagene[0].getFagnavn()};

    for (int i = 1; i <= fagene.length; i++) {
      if (fagene[i].getAntStud() > hittilStorst) {
        hittilStorst = fagene[i].getAntStud();
        storst[0] = fagene[i].getFagnavn();
      }
      if (fagene[i].getAntStud() == hittilStorst) {
        utvidTabell();
        storst[fagene.length] = fagene[i].getFagnavn();
      }
      }
      return storst;
  }
  ...
  private void utvidTabell() {
    Fag[] nyTab = new Fag[fagene.length +1];  // ny og større tabell
   ...
   */
   public static void main(String[] args) {
     ...
     boolean ok4 = kat.registrerNyttFag("LV173D", "Programmering i Java");
    ...
    */
    ...
    kat.oppdaterAntStud("LO347D",30);
    kat.oppdaterAntStud("LV173D",30);
    ...
    */
    System.out.println(kat.finnAntFag());
    System.out.println(kat.finnAntallStudenter());
